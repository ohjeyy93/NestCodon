...
for x in range(1,len(listofSamples)):
    ...
    for values in codons.values:
        count+=1
        currentGene=values[0]
        currentGene=(values[0][:-len(str(int(''.join(list(filter(str.isdigit, currentGene))))))]).replace('codon', '')
        #print(currentGene)
        if count>0:break
    currentGeneList=[]
...
for x in range(1,len(listofSamples)):
    ...
    plt.savefig("strain_"+listofSamples[x]+"_heatmap.png")
    plt.clf()
    #break
    #print(wholeGene2)
    #print(columnGene)
...
for x in range(1,len(listofSamples)):
    ...
    for gene in wholeGene2:
        ...
        #print(len(columnGene2))
        d = {"CodonPosition": columnCodonPosition2, 'CodonDepth':columnDepth2, "Genetype":columnGene2}
        df = pd.DataFrame(data=d)
